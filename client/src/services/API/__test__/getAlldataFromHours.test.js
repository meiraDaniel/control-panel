...
import getAlldataFromHours from "../getAlldataFromHours";
...
const email = 'foo'
  test(" request successfull, retrieve the response", async () => {
    ...
    await expect( getAlldataFromHours(account_id,token)).resolves.toEqual(status);
   ...
   test(" request failure, retrieve an error", async () => {
    ...
    await expect(getAlldataFromHours()).rejects.toThrow(error);
  }) 
})